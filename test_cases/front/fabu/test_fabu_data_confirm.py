...
from config import user_fabu,number


class Test_Fabu_Data(unittest.TestCase):
    """发布界面数据核对"""
    ...
    def setUp(self):
        ...
        self.process_front.into_fabu()
        self.datas = self.process_front.fabu_check(number)
    ...
    def test_1(self):
        ...
        for index, data in enumerate(self.datas):
            count = db.session.query(Fabu).filter(
                ...
                Fabu.status == 0,
                Fabu.invalidTime == data.get('invalidtime', '未找到'),
            ).count()
